...
describe("Router has correct routes", () => {
  ...
  test("has /home route", () => {
    let homeRoute = router.options.routes.filter((route) => route.path == "/home")[0];
  ...
  test("has /club-fines route", () => {
    let clubFinesRoute = router.options.routes.filter((route) => route.path == "/clubfines")[0];
  ...
  test("has / path in the router itself", () => {
    let slashRoute = router.options.routes.filter((route) => route.path == "/")[0];
  ...
  test("it contains the route and component for playerfines", () => {
    let finesRoute = router.options.routes.filter((route) => route.path == "/playerfines")[0];
  ...
  test("it contains the route and component for statistics", () => {
    let statisticsRoute = router.options.routes.filter((route) => route.path == "/statistics")[0];
